...
class Compilation {
  ...
  _processLine (lineNumber) {
    if (!this.lines[lineNumber].match(/^\s*#/)) {
      let result
      const results = []
      const reg = /%\{(\S+?)\}/g
    ...
    if (matchResult = this.lines[lineNumber].match(/^\s*#\s*$/)) return this._emptyDirective(lineNumber)
    if (matchResult = this.lines[lineNumber].match(/^\s*#include\s+\"(.+?)\"\s*$/)) return this._includeDirective(lineNumber, matchResult[1])
    if (matchResult = this.lines[lineNumber].match(/^\s*#include\s+<(.+?)>\s*$/)) return this._includeDirective(lineNumber, matchResult[1], true)
    if (matchResult = this.lines[lineNumber].match(/^\s*#define\s+(\S+)\s*$/)) return this._defineDirective(lineNumber, matchResult[1])
    if (matchResult = this.lines[lineNumber].match(/^\s*#define\s+(\S+)\s+(.+?)\s*$/)) return this._defineDirective(lineNumber, matchResult[1], matchResult[2])
    if (matchResult = this.lines[lineNumber].match(/^\s*#undef\s+(.+?)\s*$/)) return this._undefDirective(lineNumber, matchResult[1])
    if (matchResult = this.lines[lineNumber].match(/^\s*#ifdef\s+(.+?)\s*$/)) return this._ifdefDirective(lineNumber, matchResult[1])
    if (matchResult = this.lines[lineNumber].match(/^\s*#ifndef\s+(.+?)\s*$/)) return this._ifndefDirective(lineNumber, matchResult[1])
    if (matchResult = this.lines[lineNumber].match(/^\s*#if\s+(.+?)\s*$/)) return this._ifDirective(lineNumber, matchResult[1])
    if (matchResult = this.lines[lineNumber].match(/^\s*#elif\s+(.+?)\s*$/)) return this._elifDirective(lineNumber, matchResult[1])
    if (matchResult = this.lines[lineNumber].match(/^\s*#else\s*$/)) return this._elseDirective(lineNumber)
    if (matchResult = this.lines[lineNumber].match(/^\s*#endif\s*$/)) return this._endifDirective(lineNumber)
    if (matchResult = this.lines[lineNumber].match(/^\s*#error\s+(.+?)\s*$/)) return this._errorDirective(lineNumber, matchResult[1])
  ...
  _includeDirective (lineNumber, request, isLib) {
    ...
    if (filepath) {
      ...
      throw new Error(`Can not find file: "${matchResult[1]}"`)
    }
  }
